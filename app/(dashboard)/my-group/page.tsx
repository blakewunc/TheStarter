...
function computeStandings(competition: Competition) {
  ...
  for (const m of competition.matches || []) {
    ...
    if (!result) continue
    // We need to map team to side a/b — stored in match_sides, not available here
    // For Ryder Cup we'll compute cumulative points_a / points_b
    // Simple: attribute by team order (team[0]=A, team[1]=B)
      ...
      <div style={{ background: '#fff', borderRadius: '12px 12px 0 0', padding: '24px', width: '100%', maxWidth: '480px', maxHeight: '90vh', overflowY: 'auto' }}>
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <div style={{ display: 'flex', gap: '0', border: '0.5px solid rgba(28,26,23,0.15)', borderRadius: '5px', overflow: 'hidden' }}>
              {(['1v1', '2v2', 'scramble', 'stroke_play'] as const).map((f) => (
                <button
                  ...
                  style={{
                    ...
                    height: '40px',
                    border: 'none',
                    background: format === f ? '#1C1A17' : '#fff',
          ...
          {/* Players — only for non-Ryder Cup */}
          {!isRyderCup && (
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label style={labelStyle}>Side A</label>
                <select value={sideAUser} onChange={(e) => setSideAUser(e.target.value)} style={{ ...inputStyle, appearance: 'none' }}>
              ...
              <div>
                ...
                <select value={sideBUser} onChange={(e) => setSideBUser(e.target.value)} style={{ ...inputStyle, appearance: 'none' }}>
                  <option value="">Select player</option>
                  {members.map((m) => (
                    <option key={m.user_id} value={m.user_id}>
                      {memberName(m.profiles)}
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '8px' }}>
              {[
                ...
                <button
                  ...
                  style={{
                    height: '52px',
                    border: `0.5px solid ${winner === key ? '#1C1A17' : 'rgba(28,26,23,0.15)'}`,
                    borderRadius: '5px',
                    background: winner === key ? '#1C1A17' : '#fff',
                    color: winner === key ? '#F5F1ED' : '#6B6460',
                    fontSize: '12px',
                    fontWeight: 500,
                    cursor: 'pointer',
                    padding: '4px 8px',
          ...
          <button
            ...
            style={{
              width: '100%',
              height: '48px',
              background: loading || !winner ? '#A09890' : '#1C1A17',
              color: '#F5F1ED',
              border: 'none',
              borderRadius: '5px',
              fontSize: '13px',
              fontWeight: 500,
              cursor: loading || !winner ? 'not-allowed' : 'pointer',
...
function CreateGroupModal({ onClose, onCreated }: { onClose: () => void; onCreated: (g: Group) => void }) {
  ...
  return (
    <div
      style={{ position: 'fixed', inset: 0, background: 'rgba(28,26,23,0.5)', zIndex: 50, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '24px' }}
      onClick={(e) => e.target === e.currentTarget && onClose()}
    >
      <div style={{ background: '#fff', borderRadius: '8px', padding: '28px', width: '100%', maxWidth: '400px' }}>
        <h2 style={{ fontSize: '20px', fontWeight: 300, color: '#1C1A17', fontFamily: "'Cormorant Garamond', Georgia, serif", marginBottom: '20px' }}>
          Create a Group
        </h2>
        <label style={{ fontSize: '11px', fontWeight: 600, letterSpacing: '0.1em', textTransform: 'uppercase' as const, color: '#A09890', display: 'block', marginBottom: '6px' }}>
          Group Name
        </label>
        <input
          type="text"
          ...
          placeholder="e.g. The Saturday Boys"
          autoFocus
          style={{ width: '100%', height: '44px', border: '0.5px solid rgba(28,26,23,0.20)', borderRadius: '5px', padding: '0 12px', fontSize: '14px', color: '#1C1A17', background: '#fff', boxSizing: 'border-box', marginBottom: '16px' }}
        />
        <div style={{ display: 'flex', gap: '10px' }}>
          <button
            onClick={handleCreate}
            disabled={loading}
            style={{ flex: 1, height: '44px', background: '#1C1A17', color: '#F5F1ED', border: 'none', borderRadius: '5px', fontSize: '13px', fontWeight: 500, cursor: 'pointer' }}
          >
            {loading ? 'Creating...' : 'Create Group'}
          </button>
          <button
            onClick={onClose}
            style={{ height: '44px', padding: '0 20px', background: 'transparent', color: '#6B6460', border: '0.5px solid rgba(28,26,23,0.20)', borderRadius: '5px', fontSize: '13px', cursor: 'pointer' }}
          >
            Cancel
          </button>
        </div>
...
export default function MyGroupPage() {
  ...
  // Fetch full competition data (with matches) for active group
  useEffect(() => {
  ...
  if (groups.length === 0) {
    return (
      <div className="min-h-screen" style={{ background: '#F5F1ED' }}>
        <div className="mx-auto max-w-4xl px-6 py-12">
          <p style={{ fontSize: '10px', letterSpacing: '0.14em', textTransform: 'uppercase', color: '#A09890', marginBottom: '8px' }}>
            My Group
          </p>
          ...
          <div
            style={{ background: '#fff', border: '0.5px solid rgba(28,26,23,0.10)', borderRadius: '8px', padding: '48px', textAlign: 'center', maxWidth: '420px', margin: '0 auto' }}
          >
            ...
            <button
              ...
              style={{ background: '#1C1A17', color: '#F5F1ED', border: 'none', borderRadius: '5px', padding: '12px 28px', fontSize: '13px', fontWeight: 500, cursor: 'pointer', letterSpacing: '0.04em' }}
            >
              Create a Group
            </button>
          </div>
    ...
    <div className="min-h-screen" style={{ background: '#F5F1ED' }}>
      <div className="mx-auto max-w-4xl px-4 py-8 sm:px-6">
        ...
        <div style={{ marginBottom: '28px', display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between' }}>
          ...
          <button
            onClick={() => setShowCreateGroup(true)}
            style={{ background: 'transparent', border: '0.5px solid rgba(28,26,23,0.20)', borderRadius: '5px', padding: '8px 16px', fontSize: '12px', color: '#6B6460', cursor: 'pointer', whiteSpace: 'nowrap' }}
          >
            + New Group
          </button>
        </div>
            ...
            {groups.map((g) => (
              <button
                ...
                style={{
                  ...
                  background: g.id === activeGroupId ? '#1C1A17' : '#fff',
                  color: g.id === activeGroupId ? '#F5F1ED' : '#6B6460',
                  fontSize: '12px',
                  fontWeight: 500,
                  cursor: 'pointer',
                  whiteSpace: 'nowrap',
        ...
        {activeGroup && (
          <div className="space-y-6">
            ...
            {activeComp ? (
              ...
              /* No competition yet */
              <div
                style={{ background: '#fff', border: '0.5px solid rgba(28,26,23,0.10)', borderRadius: '8px', padding: '32px', textAlign: 'center' }}
              >
                ...
                <StartCompetitionButton groupId={activeGroup.id} onCreated={() => {
                  setCompetitionsData((prev) => { const copy = { ...prev }; delete copy[activeGroup.id]; return copy })
                }} />
              </div>
            )}
                  ...
                  {[...(activeComp.matches || [])]
                    ...
                    .map((match) => {
                      ...
                      return (
                        <div
                          key={match.id}
                          style={{ background: '#fff', border: '0.5px solid rgba(28,26,23,0.08)', borderRadius: '6px', padding: '12px 16px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}
                        >
                          <div>
                            <p style={{ fontSize: '13px', fontWeight: 500, color: '#1C1A17' }}>
                              {match.course || match.format}
                            </p>
                          ...
                          {result && (
                            <span
                              style={{
                                fontSize: '11px',
                                fontWeight: 500,
                                color: result.winner === 'tie' ? '#6B6460' : '#3B6D11',
                                background: result.winner === 'tie' ? 'rgba(28,26,23,0.06)' : 'rgba(59,109,17,0.08)',
                                borderRadius: '4px',
                                padding: '3px 8px',
                              }}
                            >
            ...
            <div>
              ...
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px' }}>
                {activeGroup.group_members.map((m) => (
                  <div
                    key={m.user_id}
                    style={{ background: '#fff', border: '0.5px solid rgba(28,26,23,0.08)', borderRadius: '20px', padding: '6px 14px', fontSize: '12px', color: '#1C1A17', fontWeight: 500 }}
                  >
                    {memberName(m.profiles)}
                  </div>
                ))}
...
// ─── Start Competition Button (inline) ────────────────────────────────────────

function StartCompetitionButton({ groupId, onCreated }: { groupId: string; onCreated: () => void }) {
  const [loading, setLoading] = useState(false)
    ...
    try {
      const res = await fetch(`/api/groups/${groupId}/competitions`, {
        ...
        body: JSON.stringify({
          ...
          format: 'ryder_cup',
          teams: [
            { name: 'Team A', color: '#70798C' },
            { name: 'Team B', color: '#B5A98A' },
          ],
        }),
      })
      if (!res.ok) throw new Error((await res.json()).error)
  ...
  return (
    ...
    >
      {loading ? 'Starting...' : 'Start a Ryder Cup Season'}
    </button>
  )
}
