...
export default function AdminDashboard() {
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto', padding: '40px 24px' }}>
      <h1 style={{ fontSize: '26px', fontWeight: 300, color: '#1C1A17', fontFamily: "var(--serif)", marginBottom: '32px' }}>
        Dashboard
      </h1>
        ...
        {cards.map(({ label, value, href }) => (
          <Link key={label} href={href} style={{ textDecoration: 'none' }}>
            <div style={{ background: '#fff', border: '0.5px solid rgba(28,26,23,0.10)', borderRadius: '8px', padding: '20px' }}>
              ...
              <p style={{ fontSize: '28px', fontWeight: 300, color: '#1C1A17', fontFamily: "var(--serif)" }}>{value}</p>
            </div>
          </Link>
        ))}
      </div>
