...
export default function AdminDashboard() {
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto', padding: '40px 24px' }}>
      <h1 style={{ fontSize: '26px', fontWeight: 300, color: '#1C1A17', fontFamily: "'Playfair Display', Georgia, serif", marginBottom: '32px' }}>
        Dashboard
      </h1>
        ...
        {cards.map(({ label, value, href }) => (
          <Link key={label} href={href} style={{ textDecoration: 'none' }}>
            <div style={{ background: '#fff', border: '0.5px solid rgba(28,26,23,0.10)', borderRadius: '8px', padding: '20px' }}>
              <p style={{ fontSize: '10px', letterSpacing: '0.10em', textTransform: 'uppercase', color: '#6B6460', marginBottom: '8px' }}>{label}</p>
              <p style={{ fontSize: '28px', fontWeight: 300, color: '#1C1A17', fontFamily: "'Playfair Display', Georgia, serif" }}>{value}</p>
            </div>
          </Link>
        ))}
      </div>
      ...
      {/* Quick actions */}
      <p style={{ fontSize: '10px', letterSpacing: '0.12em', textTransform: 'uppercase', color: '#6B6460', marginBottom: '12px', fontWeight: 600 }}>Quick Actions</p>
          ...
          <Link key={href} href={href} style={{ textDecoration: 'none' }}>
            <div style={{ background: '#fff', border: '0.5px solid rgba(28,26,23,0.10)', borderRadius: '8px', padding: '16px 20px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              <div>
                ...
                <p style={{ fontSize: '12px', color: '#6B6460' }}>{desc}</p>
              </div>
              <span style={{ fontSize: '14px', color: '#3B6D11' }}>→</span>
            </div>
          </Link>
        ))}
      </div>
